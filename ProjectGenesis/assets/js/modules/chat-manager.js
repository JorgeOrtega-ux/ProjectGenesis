...
// --- ▼▼▼ MODIFICACIÓN (MENÚ DE ACCIONES DE MENSAJE A POPOVER) ▼▼▼ ---
        ...
        if (query.length > 0) {
            ...
            switch (currentChatFilter) {
                ...
                default:
                    ...
                    emptyText.dataset.i1a8n = 'chat.empty.all';
                    break;
            }
        } else {
...
// --- ▼▼▼ INICIO DE MODIFICACIÓN (createMessageBubbleHtml) ▼▼▼ ---
    ...
    if (msg.status !== 'deleted') {
         actionsMenuHtml = `
            <div class="chat-bubble-actions">
                <button type="button" class="chat-action-btn" data-action="toggle-message-context-menu" title="Más opciones">
...
// --- ▲▲▲ FIN DE MODIFICACIÓN (createMessageBubbleHtml) ▲▲▲ ---
...
export function initChatManager() {
    ...
    document.body.addEventListener('click', async (e) => {
        ...
        if (!chatSection) {
            // Si el clic fue FUERA de la sección de chat, cerrar los popovers si existen
            if (chatPopperInstance) {
                chatPopperInstance.destroy();
                chatPopperInstance = null;
            }
            // --- ▼▼▼ INICIO DE MODIFICACIÓN (CERRAR POPOVER DE MENSAJE) ▼▼▼ ---
            if (messagePopperInstance) {
                messagePopperInstance.destroy();
                messagePopperInstance = null;
            }
            // --- ▲▲▲ FIN DE MODIFICACIÓN ▲▲▲ ---
        ...
        // --- ▼▼▼ INICIO DE MODIFICACIÓN (NUEVOS LISTENERS DE POPOVER) ▼▼▼ ---
        
        // 2b. Comprobar si se hizo clic en el BOTÓN DEL MENÚ DE UN MENSAJE
        const messageContextBtn = e.target.closest('[data-action="toggle-message-context-menu"]');
        if (messageContextBtn) {
            e.preventDefault();
            e.stopImmediatePropagation();

            const messageBubble = messageContextBtn.closest('.chat-bubble');
            const popover = document.getElementById('message-context-menu'); // Asume que existe en messages.php

            if (!messageBubble || !popover) {
                console.error("No se encontró el globo de chat o el popover #message-context-menu");
                return;
            }
            ...
            // Cerrar el otro popover (el de la lista) si está abierto
            if (chatPopperInstance) {
                chatPopperInstance.destroy();
                chatPopperInstance = null;
            }
            ...
            // Guardar datos en el popover para que los usen los botones de acción
            popover.dataset.messageId = messageBubble.dataset.messageId;
            popover.dataset.textContent = messageBubble.dataset.textContent;
            
            const isSent = messageBubble.classList.contains('sent');
            
            // Configurar visibilidad de botones en el popover
            popover.querySelector('[data-action="msg-reply"]').style.display = 'flex';
            popover.querySelector('[data-action="msg-copy"]').style.display = messageBubble.dataset.textContent ? 'flex' : 'none';
            popover.querySelector('[data-action="msg-delete"]').style.display = isSent ? 'flex' : 'none';

            messagePopperInstance = createPopper(messageContextBtn, popover, {
        ...
        // 2c. Comprobar si se hizo clic DENTRO del popover DE MENSAJE
        const messagePopoverOption = e.target.closest('#message-context-menu .menu-link');
        if (messagePopoverOption) {
             e.preventDefault();
             e.stopPropagation();
             
             if (messagePopoverOption.disabled) return; 
             
             const action = messagePopoverOption.dataset.action;
             const popover = messagePopoverOption.closest('#message-context-menu');
             const messageId = popover.dataset.messageId;
             const textContent = popover.dataset.textContent;
             const messageBubble = document.querySelector(`.chat-bubble[data-message-id="${messageId}"]`);
             ...
             if (action === 'msg-copy') {
                 ...
                 if (result.success) {
                     ...
                     // El handler de WS (handleMessageDeleted) se encargará de la UI
                 } else {
                     showAlert(getTranslation(result.message || 'js.api.errorServer'), 'error');
                 }
             } 
        ...
        // --- ▲▲▲ FIN DE MODIFICACIÓN (NUEVOS LISTENERS DE POPOVER) ▲▲▲ ---
        ...
        // --- ▼▼▼ INICIO DE MODIFICACIÓN (CERRAR POPOVERS) ▼▼▼ ---
        ...
        const clickedOnMessagePopover = e.target.closest('#message-context-menu.active'); // <-- NUEVO
        
        if (!contextBtn && !messageContextBtn && !clickedOnPopover && !clickedOnMessagePopover) { // <-- MODIFICADO
             if (chatPopperInstance) {
                chatPopperInstance.destroy();
                chatPopperInstance = null;
             }
             if (messagePopperInstance) { // <-- NUEVO
                messagePopperInstance.destroy();
                messagePopperInstance = null;
             }
             document.querySelector('.chat-item-menu-badge.popover-active')?.classList.remove('popover-active');
        }
        // --- ▲▲▲ FIN DE MODIFICACIÓN (CERRAR POPOVERS) ▲▲▲ ---
        ...
        // --- ▼▼▼ INICIO DE MODIFICACIÓN (ELIMINACIÓN DE LÓGICA DE BOTONES) ▼▼▼ ---
        const actionBtn = e.target.closest('.chat-action-btn[data-action]');
        if (actionBtn) {
            const action = actionBtn.dataset.action;
            
            // Las acciones 'msg-copy', 'msg-reply', 'msg-delete' y 'toggle-message-context-menu'
            // se manejan en bloques anteriores.
            
            // Solo mantenemos acciones no migradas, como 'msg-info'
            if (action === 'msg-info') {
        ...
        // --- ▲▲▲ FIN DE MODIFICACIÓN (ELIMINACIÓN DE LÓGICA DE BOTONES) ▲▲▲ ---
    });

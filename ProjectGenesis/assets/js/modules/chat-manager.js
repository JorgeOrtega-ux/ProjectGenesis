...
let messagePopperInstance = null; 
...
function renderConversationList(conversations) {
    ...
    conversations.forEach(convo => {
        ...
        const isBlocked = convo.is_blocked_globally === true; 
...
async function openChat(targetId, name, avatar, role, isOnline, lastSeen) {
    ...
    currentChatUserId = parseInt(targetId, 10); 
...
async function sendMessage() {
    ...
    formData.append('target_id', currentChatTargetId); 
    ...
    try {
        ...
        if (result.success && result.message_sent) {
            ...
            if (result.message === 'js.chat.errorBlocked' || 
                ...
                result.message === 'js.chat.errorPrivacyNone' || 
                result.message === 'js.chat.errorPrivacyFriendsOnly' 
               ) {
...
export function initChatManager() {
    ...
    document.body.addEventListener('click', async (e) => {
        ...
        const clickedOnMessagePopover = e.target.closest('#message-context-menu.active'); 
        
        if (!contextBtn && !messageContextBtn && !clickedOnPopover && !clickedOnMessagePopover) { 
             if (chatPopperInstance) {
                chatPopperInstance.destroy();
                chatPopperInstance = null;
             }

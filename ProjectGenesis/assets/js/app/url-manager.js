...
const paths = {
    ...
    '/messages/uuid-placeholder': 'toggleSectionMessages',
...
export function handleNavigation() {
    ...
    const messagesRegex = /^\/messages\/([a-fA-F0-9\-]{36})$/i;
...
export function initRouter() {
    ...
    document.body.addEventListener('click', e => {
        ...
        const link = e.target.closest(
            ...
            'a[href*="/profile/"], a[href*="/search"], a[href*="/messages"], ' +
            '.component-button[data-action*="toggleSection"], ' +
            '.header-button[data-action*="toggleSection"], ' +
            ...
            'a.chat-conversation-item[data-nav-js="true"]'
        );
        ...
        if (link) {
            ...
            if (link.hasAttribute('data-action')) {
                ...
                if (action === 'friend-menu-message' && link.dataset.uuid) {
                    const userUuid = link.dataset.uuid;
                    page = 'messages';
                    action = 'toggleSectionMessages';
                    newPath = '/messages/' + userUuid;
                    fetchParams = { user_uuid: userUuid };
                }
                ...
                if (postViewRegex.test(newPath)) {
                    ...
                    fetchParams = { user_uuid: matches[1] };
